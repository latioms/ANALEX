	...
	public Automate evaluation(List<Stockage> ls){
		...
		for(Stockage stock:ls) {
			...
			//si on est en presence d'un operateur
				...
				else if(stock.getValeur()=='|' || stock.getValeur()=='+') {
					Automate auto_symb1=pile.pop();
					Automate auto_symb2=pile.pop();
					//creation de automate par concatenation
...
//					}while(pile.isEmpty()||pile.pop().getValeur()=='(');
	...
	public Automate union(Automate a1, Automate a2) {
		...
		Etat_transition etat_fin=new Etat_transition(new ArrayList<Transition>(), true);
		//creation des transitions vers etats finals crÃ©e
	...
	//	mettre les etats finaux de a1 et a2 Ã  false
	...
	public Automate multiplicite(Automate a) {
		...
		Transition t3=new Transition('£',etat_fin);
		//cette epsilonne transition part de l'ancien etat final Ã  ancien etat initial
...
//		a la fin on ajoute les deux etats debut et fin Ã  la liste des etats.
	...
	public Etat_transition etat_final_ele(Automate a) {
        ...
        ***** ensemble de methodes implementÃ© par ramses
   ...
   //retourne une liste de stockage     
	...
	public boolean verification(String chaine) {
            ...
            for(int i=0; i<chaine.length(); i++){
                if(i==0 && ((chaine.charAt(i)=='|' || chaine.charAt(i)=='+') || chaine.charAt(i)=='*'))
                    return false;
            
                if(i==chaine.length()-1 && (chaine.charAt(i)=='|' || chaine.charAt(i)=='+'))
                    return false;
                ...
                if((chaine.charAt(i)=='|' || chaine.charAt(i)=='+') && ((chaine.charAt(i+1)=='|' || chaine.charAt(i+1)=='+') || chaine.charAt(i+1)=='*')){
                    return false;
                }
	...
	public boolean isOperateur(char valeur) {
		// TODO Auto-generated method stub
                if(valeur=='*'||valeur=='|'||valeur=='.'|| valeur=='('||valeur==')'||valeur=='+')
			return true;
		return false;
	}
...
//                    Transition t=new Transition('Â£', a);
//                    liste.add(t);
                }
                if(isInto(detat, u)==null){
